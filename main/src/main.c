...
int main()
{
    ...
    while (1)
    {
        if (ETH_get_receive_flag() == 1)
        {
            ...
            uint32_t cnt_msg = 0, valid_delay=0, period=0;
            ...
            {
                cnt_msg=D_MAX_UINT32; // put cnt_msg to max int32 to avoid send message because of wrong input delay
            }
            TIMER_init(&TIMER_Bank->TIM2, prescaler, D_MAX_UINT32); /* Initialize timer to count by ms*/
            TIMER_get_tick_period(command.delay,&period);
            ...
            { 
                ...
                if ((TIMER_get_value(&TIMER_Bank->TIM2) - time_send) >= period)  /* Check if the frame has been sent and if 100 ticks has passed delay of 100 ticks depends on frame input*/
                {
